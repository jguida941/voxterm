...
> **Scheduling note**: This document is a design/roadmap reference. Active priorities and execution live in `dev/active/MASTER_PLAN.md`.
...
| **Full TUI** (`src`) | Ratatui + Crossterm | 3-panel layout, red theme, rounded borders |
...
**Files**: NEW `src/src/backend/mod.rs`, `src/src/backend/claude.rs`, `src/src/backend/gemini.rs`, etc.
...
**Status**: Terminal restore guard, minimal crash log, `--doctor`, and resize clear completed on 2026-02-01 (see `dev/archive/2026-02-01-terminal-restore-guard.md`). Remaining items below.
...
**Files**: `src/src/terminal_restore.rs`, `src/src/doctor.rs`, `src/src/app/logging.rs`
...
**Files**: `src/src/bin/codex_overlay/theme.rs`, `src/src/bin/codex_overlay/config.rs`, `src/src/bin/codex_overlay/main.rs`, NEW `src/src/accessibility.rs`
...
**Files**: `src/src/bin/codex_overlay/writer.rs`, NEW `src/src/render_diff.rs`, `src/src/perf_metrics.rs`, `src/src/perf_overlay.rs`, `src/src/hud_metrics.rs`
...
**Files**: NEW `src/src/error_overlay.rs`, `src/src/pipeline_overlay.rs`
...
**Files**: NEW `src/src/hud_widgets.rs`, `src/src/hud_metrics.rs`
...
**Files**: NEW `src/src/preferences.rs`, `src/src/bin/codex_overlay/main.rs`, `src/src/bin/codex_overlay/config.rs`
...
**Files**: NEW `src/src/config_migrate.rs`, `src/src/preferences.rs`
...
**Files**: `src/src/bin/codex_overlay/theme.rs`
...
**Files**: NEW `src/src/keybindings.rs`, `src/src/bin/codex_overlay/config.rs`, `src/src/bin/codex_overlay/input.rs`
...
**Files**: NEW `src/src/text_editor.rs`
...
For phases that change architecture or persistence, write an ADR in `dev/adr/`
...
Update the ADR index in `dev/adr/README.md` when adding new records.

---

## Implementation Roadmap

**Priority note**: The ordering below is conceptual; active priorities and scheduling are tracked in `dev/active/MASTER_PLAN.md`.
...
- [ ] Dependencies tracked in `dev/active/MASTER_PLAN.md`
...
- [ ] `dev/CHANGELOG.md` updated for user-facing changes
- [ ] `dev/active/MASTER_PLAN.md` updated; completed items moved to `dev/archive/`
...
- [ ] If releasing: update changelog + bump version; maintain local review notes in `dev/active/` (gitignored)
...
**Files**: NEW `src/src/markdown.rs`, `src/src/syntax.rs`
...
**Files**: NEW `src/src/graphics.rs`, `src/src/image_render.rs`
...
**Files**: NEW `src/src/mcp_client.rs`, `src/src/mcp_config.rs`
...
| File | Purpose |
|------|---------|
| `src/src/bin/codex_overlay/status_line.rs` | Multi-row status banner layout |
| `src/src/bin/codex_overlay/status_style.rs` | Message styling, status types |
| `src/src/bin/codex_overlay/theme.rs` | Color palettes, border sets |
| `src/src/bin/codex_overlay/theme_picker.rs` | Theme selection overlay |
| `src/src/bin/codex_overlay/help.rs` | Help overlay |
| `src/src/bin/codex_overlay/banner.rs` | Startup banner |
| `src/src/bin/codex_overlay/audio_meter.rs` | Waveform visualization |
| `src/src/bin/codex_overlay/writer.rs` | Terminal I/O, rendering |
| `src/src/bin/codex_overlay/main.rs` | Main loop, input handling |
| `src/src/bin/codex_overlay/config.rs` | CLI flags, configuration |
| `src/src/bin/codex_overlay/session_stats.rs` | Exit statistics |
...
| File | Purpose | Phase |
|------|---------|-------|
| `src/src/backend/mod.rs` | AiBackend trait + registry | -2 |
| `src/src/backend/claude.rs` | Claude Code backend | -2 |
| `src/src/backend/gemini.rs` | Gemini CLI backend | -2 |
| `src/src/backend/aider.rs` | Aider backend | -2 |
| `src/src/backend/opencode.rs` | OpenCode backend | -2 |
| `src/src/backend/custom.rs` | Custom command backend | -2 |
| `src/src/hud/mod.rs` | HUD module trait + registry | -2 |
| `src/src/hud/mode.rs` | Mode indicator module | -2 |
| `src/src/hud/meter.rs` | Audio meter module | -2 |
| `src/src/hud/latency.rs` | Latency display module | -2 |
| `src/src/hud/queue.rs` | Queue depth module | -2 |
| `src/src/icons.rs` | Icon vocabulary (Unicode/ASCII) | -2 |

**Phase -1 to 9.5 (Core Roadmap)**:
| File | Purpose | Phase |
|------|---------|-------|
| `src/src/terminal_restore.rs` | Terminal state guard + panic restore | -1 |
| `src/src/crash_log.rs` | Crash log + event ring buffer | -1 |
| `src/src/doctor.rs` | `voxterm --doctor` diagnostics | -1 |
| `src/src/privacy.rs` | Redaction policy + retention settings | -1 |
| `src/src/sanitize.rs` | Control-char stripping + safe render helpers | -1 |
| `src/src/accessibility.rs` | Announcements, contrast validation, reduced motion | 0 |
| `src/src/render_diff.rs` | Dirty-line diff rendering | 0.5 |
| `src/src/perf_metrics.rs` | Render/flush timing metrics | 0.5 |
| `src/src/perf_overlay.rs` | Perf debug overlay | 0.5 |
| `src/src/event_coalesce.rs` | Update coalescing for high-frequency signals | 0.5 |
| `src/src/hud_layout.rs` | Segment layout engine + fit helpers | 0.5 |
| `src/src/hud_metrics.rs` | HUD metrics collection/aggregation | 0.5 |
| `src/src/focus.rs` | FocusManager, focus trap, restoration | 1 |
| `src/src/overlay_stack.rs` | Overlay stack + modal routing | 1 |
| `src/src/ui_primitives/` | Shared UI primitives (menus, focus, actions, layout) | 1 |
| `src/src/ui_core/` | Headless state/actions/reducer/dirty flags | 1 |
| `src/src/selectable_menu.rs` | Reusable arrow-key menu component | 2 |
| `src/src/error_overlay.rs` | Error details modal | 2.5 |
| `src/src/pipeline_overlay.rs` | Pipeline activity overlay | 2.5 |
| `src/src/hud_widgets.rs` | HUD widgets (model/latency/context/queue) | 2.6 |
| `src/src/backend_metadata.rs` | Backend metadata contract (optional fields) | 2.6 |
| `src/src/preferences.rs` | Config file load/save | 3 |
| `src/src/config_migrate.rs` | Config versioning + migrations | 3.2 |
| `src/src/keybindings.rs` | Keybinding config load/save/merge | 3.6 |
| `src/src/actions.rs` | Action registry (keys/help/palette/settings) | 3.6 |
| `src/src/design_tokens.rs` | Token spec + contrast checks | 3.5 |
| `src/src/settings.rs` | Settings overlay | 4 |
| `src/src/calibration_overlay.rs` | Audio calibration flow | 4.2 |
| `src/src/text_editor.rs` | Minimal text editor widget | 4.7 |
| `src/src/command_palette.rs` | Command palette overlay | 5 |
| `src/src/history.rs` | Transcript history | 7 |
| `src/src/outbox.rs` | Outbox / retry queue | 7.5 |
| `src/src/toast.rs` | Toast notification system | Micro |
| `.github/workflows/terminal_matrix.yml` | Terminal compatibility smoke tests | 9.5 |

**Phase 10-20 (Future Vision)**:
| File | Purpose | Phase |
|------|---------|-------|
| `src/src/markdown.rs` | Markdown parsing and rendering | 10 |
| `src/src/syntax.rs` | Syntax highlighting for code blocks | 11 |
| `src/src/graphics.rs` | Terminal graphics protocol detection | 15 |
| `src/src/image_render.rs` | Sixel/Kitty/iTerm2 image rendering | 15 |
| `src/src/mcp_client.rs` | MCP protocol client implementation | 12 |
| `src/src/mcp_config.rs` | MCP server configuration | 12 |
| `src/src/model_router.rs` | Multi-model routing and selection | 13 |
| `src/src/local_llm.rs` | Local LLM integration (Ollama) | 14 |
| `src/src/macros.rs` | Voice macros and snippets | 16 |
| `src/src/analytics.rs` | Session analytics and stats | 17 |
| `src/src/tts.rs` | Text-to-speech response output | 18 |
| `src/src/ambient.rs` | Ambient notifications system | 19 |
